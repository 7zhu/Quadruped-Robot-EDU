...
def track_object(det,distance=0, image_size=[640,480], prev_center=None, prev_frame_time=0, prev_angle=0):
    """
    跟踪目标物体并计算前进距离和角速度
    det: 当前帧的检测结果，格式为[xmin, ymin, xmax, ymax, conf, cls]
    im0: 原始图像
    prev_center: 上一帧的目标中心点坐标，格式为(x, y)
    prev_frame_time: 上一帧的时间戳
    prev_angle: 上一帧计算得到的角度
    """
    if len(det) == 0:
    ...
    *xyxy, _, _ = det[0]  # 取第一个检测框
    ...
    if distance > 1 :
        forward_amount = distance * distance_factor
    else :
        forward_amount = 0
    # 更新prev_center
    ...
    return forward_amount, -angle_velocity, new_center, new_frame_time, prev_angle
