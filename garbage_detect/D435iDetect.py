...
# 检测的主题函数
#==========================================================================================#
def detect(opt, yuyin_chat, save_img=False):
    # 加载参数
    out, source, weights, view_img, save_txt, imgsz, half, need_beep = \
        opt.save_dir, opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, opt.half, opt.beep
    ...
    while True:
        # print(num_deep_frames)
        num_deep_frames = num_deep_frames + 1
        if num_deep_frames == 18:
            num_deep_frames = 0
            if deep_frames_flag:
