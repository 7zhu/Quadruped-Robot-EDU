    ...
    while True:
        ...
        if result.multi_hand_landmarks:
            for handLms in result.multi_hand_landmarks:
                # 绘制手部关键点和骨骼
                drawing_utils.draw_landmarks(image, handLms, mp_hands.HAND_CONNECTIONS)
                ...
                if not gesture_detector.hand_open_detected:
                    ...
                    # 如果5秒内没有执行任何手势，重置 hand_open_detected
                    ...
                    # 检测手势1、手势2和翻转，仅在 hand_open_detected 为 True 时检测
                    if gesture_detector.process_flip(handLms.landmark) and gesture_last_sent != "flip_detected":
                        print("Flip detected")
                        pipe.send("flip_detected")

                        gesture_detector.hand_open_detected = False  # 重置五指张开状态
                        gesture_detector.hand_open_detected_time = 0  # 重置五指张开计时器
